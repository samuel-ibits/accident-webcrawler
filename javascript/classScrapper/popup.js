document.addEventListener('DOMContentLoaded', function () {
    ...
    const filterResultsByCategory = (results, category) => {
        ...
        return results.texts.filter(text => text.toLowerCase().includes(lowerCategory));
    };
    ...
    const validateInput = () => {
        ...
        const isAccidentTypeSelected = accidentTypeSelect.value !== 'Select Accident Type';
    ...
    const checkClass = async () => {
        if (validateInput()) {
            ...
            chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
                chrome.tabs.sendMessage(tabs[0].id, { action: 'checkClass', className: className, elementType: elementType, accidentType: accidentType });
            });
    ...
    const saveResults = () => {
        if (resultsData) {
            ...
            if (filteredResults.length > 0) {
                ...
                if (fileName) {
                    let blob = new Blob([JSON.stringify({ texts: filteredResults })], { type: 'application/json' });
