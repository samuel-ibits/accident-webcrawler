...
async function sendToApi(data,categoryid) {
  console.log("data form scrape and about to be sent to temp db", data, categoryid)

  const {accidentType,date,details,location} = data;
  const newData={
    accidentType,
    location,
...
async function runPuppeteerTask(formdata) {
    const {searchBase, emergencyType,specialParameters, startDate, endDate,categoryid} = formdata;
const query= emergencyType + specialParameters;
  ...
  try {
    ...
    reports.forEach((items) => sendToApi(items,categoryid));
